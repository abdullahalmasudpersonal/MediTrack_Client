 ...
 <div>
  <div style="margin-bottom: 30px">
   ...
   <div class="admin-profile-top-part-div2">
    <img :src="singleDoctor?.photo || default_profile_img" />
    <div>
     <p class="admin-name">{{ singleDoctor?.name || 'Loading...' }}</p>
   ...
   <v-row>
    ...
    <v-col cols="12" md="6">
     <v-row class="text-white">
      ...
      <v-col cols="6">{{ singleDoctor?.name }}</v-col>

      <v-col cols="6" class="font-weight-bold">Email</v-col>
      <v-col cols="6">{{ singleDoctor?.user?.email }}</v-col>

      <v-col cols="6" class="font-weight-bold">Address</v-col>
      <v-col cols="6">{{ singleDoctor?.address }}</v-col>

      <v-col cols="6" class="font-weight-bold">Role</v-col>
      <v-col cols="6">{{ singleDoctor?.user?.role }}</v-col>
     </v-row>
    </v-col>
    ...
    <v-col cols="12" md="6">
     <v-row class="text-white">
      ...
      <v-col cols="6">{{ singleDoctor?.user?.userId }}</v-col>

      <v-col cols="6" class="font-weight-bold">Contact</v-col>
      <v-col cols="6">{{ singleDoctor?.phone_number }}</v-col>

      <v-col cols="6" class="font-weight-bold">Status</v-col>
      <v-col cols="6">{{ singleDoctor?.user?.status }}</v-col>

      <v-col cols="6" class="font-weight-bold">Join</v-col>
      <v-col cols="6">{{ singleDoctor?.user?.created_at }}</v-col>
     </v-row>
    </v-col>
   </v-row>
  </v-card>

  <v-card class="pa-6 mt-6" color="#002B50">
   <v-card-title class="text-h6 text-white mb-4" style="padding-left: 0; font-weight: bold">
    Doctor Schedule
   </v-card-title>
  </v-card>
